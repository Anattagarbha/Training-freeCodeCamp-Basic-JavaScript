...
In the casino game Blackjack, a player can determine whether they have 
an advantage on the next hand over the house by keeping track of the relative 
...
function cc(card) {
  ...
  if (card > 1 && card < 7) {
    count++;
  } else if (card === 10 || symb.test(card)) {
    count--;
  }
  if (count > 0) {
    return count + " Bet";
